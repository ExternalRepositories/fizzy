...
TEST(execute_call, imported_function_call)
{
    ...
    */
    ...
    constexpr auto host_foo = [](Instance&, const Value*, int) -> ExecutionResult {
        return Value{42};
    };
...
TEST(execute_call, imported_function_call_with_arguments)
{
    ...
    */
    ...
    constexpr auto host_foo = [](Instance&, const Value* args, int) -> ExecutionResult {
...
TEST(execute_call, imported_functions_call_indirect)
{
    ...
    */
    ...
    constexpr auto sqr = [](Instance&, const Value* args, int) -> ExecutionResult {
        const auto x = args[0].i32;
        return Value{uint64_t{x} * uint64_t{x}};
    };
    constexpr auto isqrt = [](Instance&, const Value* args, int) -> ExecutionResult {
        const auto x = args[0].i32;
...
TEST(execute_call, imported_function_with_cotext)
{
    /* wat2wasm
    (import "mod" "foo" (func))
    (func
...
TEST(execute_call, imported_function_with_capture)
{
    ...
    */
    ...
    int counter = 0;
    const auto host_func = [&counter](Instance&, const Value*, int) -> ExecutionResult {
        ++counter;
        return Void;
    };
    const auto& host_func_type = module->typesec[0];
...
TEST(execute_call, call_initial_depth)
{
    ...
    */
    ...
    constexpr auto host_foo = [](Instance&, const Value*, int depth) {
        EXPECT_EQ(depth, 0);
        return Void;
    };
    const auto host_foo_type = module->typesec[0];
